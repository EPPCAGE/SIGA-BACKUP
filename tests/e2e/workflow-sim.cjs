...
<definitions xmlns="http://www.omg.org/spec/BPMN/20100524/MODEL" xmlns:bpmndi="http://www.omg.org/spec/BPMN/20100524/DI" xmlns:omgdc="http://www.omg.org/spec/DD/20100524/DC" xmlns:omgdi="http://www.omg.org/spec/DD/20100524/DI" targetNamespace="http://ep.cage">
  ...
  const sdk = {
    ...
    addDoc: async (ref, data) => {
      const id = `id_${++seq}`;
      ensureCollection(ref.col).set(id, { ...data, _id: id });
      return { id };
    },
    getDoc: async (ref) => {
      const data = ensureCollection(ref.col).get(ref.id);
      return {
        id: ref.id,
        exists: () => !!data,
        data: () => data,
    ...
    updateDoc: async (ref, data) => {
      const col = ensureCollection(ref.col);
    ...
    deleteDoc: async (ref) => {
      ensureCollection(ref.col).delete(ref.id);
    },
  };
...
(async () => {
  ...
  const result = await page.evaluate(runWorkflowSimulation);
